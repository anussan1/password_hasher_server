...
func parseURLParams(req *http.Request, bodyData *[]byte) (url.Values, error) {
	...
	// this will  take input via url parameters or body data
...
// request handler for any other end points than the predefind ones
...
// compute base64encode hash of a new password if provided
...
// Returns the encoded password corresponding with the ID provided (if available)
...
func makeFunc_handleHashRequest(w http.ResponseWriter, req *http.Request) {
	...
	case "GET":
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("The request id should  be an integer, but got '%s'\n", reqID), http.StatusBadRequest)
			return
		}
		...
		if passwordHashbase64Available {
			...
			http.NotFound(w, req) // no pattern matched  - send 404 response
			return
		}
...
func handleStatsRequest(w http.ResponseWriter, req *http.Request) {
	...
	serverStats.RLock()
	stats := struct {
		Total   int     "total" // json output structure
...
func main() {
	...
	http.HandleFunc("/", handleGeneralRequest) // the default handler just returns "404 page not found"
